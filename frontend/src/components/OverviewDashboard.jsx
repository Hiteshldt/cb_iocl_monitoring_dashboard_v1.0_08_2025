...
const getCachedValues = () => {
  try {
    const cached = localStorage.getItem('accumulatedValues');
    if (cached) {
  ...
  return { co2AbsorbedGrams: 0, o2GeneratedLiters: 0 };
};

// Helper to save values to localStorage
const saveCachedValues = (values) => {
  try {
    localStorage.setItem('accumulatedValues', JSON.stringify(values));
  } catch (e) {
    // Ignore localStorage errors
  }
};

const OverviewDashboard = ({ data, relayNames, onAirflowUpdate }) => {
  ...
  // Update previous values when we get valid data
  useEffect(() => {
    const newCo2 = data?.calculated?.co2?.absorbedGrams;
    const newO2 = data?.calculated?.o2?.generatedLiters;

    // Only update if we have valid numbers (not undefined, null, or NaN)
    if (typeof newCo2 === 'number' && !isNaN(newCo2) && newCo2 >= prevValuesRef.current.co2AbsorbedGrams) {
      prevValuesRef.current.co2AbsorbedGrams = newCo2;
    }
    if (typeof newO2 === 'number' && !isNaN(newO2) && newO2 >= prevValuesRef.current.o2GeneratedLiters) {
      prevValuesRef.current.o2GeneratedLiters = newO2;
    }
  ...
  const { calculated, relays } = data;

  // Use current values if valid (not undefined/null/NaN), otherwise use cached previous values
  const rawCo2 = calculated?.co2?.absorbedGrams;
  const rawO2 = calculated?.o2?.generatedLiters;

  const co2AbsorbedGrams = (typeof rawCo2 === 'number' && !isNaN(rawCo2) && rawCo2 > 0)
    ? rawCo2
    : prevValuesRef.current.co2AbsorbedGrams;

  const o2GeneratedLiters = (typeof rawO2 === 'number' && !isNaN(rawO2) && rawO2 > 0)
    ? rawO2
    : prevValuesRef.current.o2GeneratedLiters;
  ...
  const aqiInfo = getAqiColor(calculated?.aqi?.value || 0);

  // Format CO2 grams to readable format
  const formatCO2 = (grams) => {
    if (grams < 1) return { value: (grams * 1000).toFixed(2), unit: 'mg' };
    if (grams < 1000) return { value: grams.toFixed(2), unit: 'g' };
    return { value: (grams / 1000).toFixed(3), unit: 'kg' };
  };

  // Format O2 liters to readable format
  const formatO2 = (liters) => {
    if (liters < 0.001) return { value: (liters * 1000).toFixed(2), unit: 'mL' };
    if (liters < 1) return { value: (liters * 1000).toFixed(1), unit: 'mL' };
    return { value: liters.toFixed(3), unit: 'L' };
  };
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className={`rounded-lg border overflow-hidden ${isDark ? 'bg-slate-900 border-slate-700' : 'bg-white border-gray-200 shadow-sm'}`}>
          ...
          <div className="px-4 py-3">
            ...
            <div className={`mt-2 h-1.5 rounded-full ${isDark ? 'bg-slate-700' : 'bg-gray-200'}`}>
              <div className={`h-full rounded-full ${aqiInfo.bg}`} style={{ width: `${Math.min((calculated?.aqi?.value || 0) / 300 * 100, 100)}%` }} />
            </div>
          </div>
        </div>
          ...
          <div className="px-4 py-3">
            <div className="flex items-baseline space-x-1">
              <span className={`text-3xl font-bold tabular-nums ${isDark ? 'text-white' : 'text-gray-900'}`}>
                {(calculated?.temperature || 0).toFixed(1)}
              </span>
        ...
        <div className={`rounded-lg border overflow-hidden ${isDark ? 'bg-slate-900 border-slate-700' : 'bg-white border-gray-200 shadow-sm'}`}>
          ...
          <div className="px-4 py-3">
            <div className="flex items-baseline space-x-1">
              <span className={`text-3xl font-bold tabular-nums ${isDark ? 'text-white' : 'text-gray-900'}`}>
                {(calculated?.humidity || 0).toFixed(1)}
              </span>
        ...
        <div className={`rounded-lg border overflow-hidden ${isDark ? 'bg-slate-900 border-slate-700' : 'bg-white border-gray-200 shadow-sm'}`}>
          ...
          <div className="px-4 py-3">
            <div className="flex items-baseline space-x-1">
              <span className={`text-3xl font-bold tabular-nums ${isDark ? 'text-white' : 'text-gray-900'}`}>
                {Math.max(0, calculated?.co2?.difference || 0).toFixed(1)}
              </span>
              <span className={`text-lg ${isDark ? 'text-slate-400' : 'text-gray-500'}`}>ppm</span>
            </div>
            <p className={`text-xs mt-1 ${isDark ? 'text-slate-500' : 'text-gray-500'}`}>Inlet - Outlet</p>
          </div>
        </div>
      </div>
            ...
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="text-right">
                <span className={`text-2xl font-bold tabular-nums ${isDark ? 'text-emerald-400' : 'text-emerald-600'}`}>
                  {formatCO2(co2AbsorbedGrams).value}
                </span>
                <span className={`text-sm ml-1 ${isDark ? 'text-emerald-500' : 'text-emerald-700'}`}>
                  {formatCO2(co2AbsorbedGrams).unit}
                </span>
              </div>
            </div>
            <div className={`h-2 rounded-full ${isDark ? 'bg-slate-700' : 'bg-gray-200'}`}>
              <div
                className="h-full rounded-full bg-gradient-to-r from-emerald-600 to-green-500 transition-all"
                style={{ width: `${Math.min(co2AbsorbedGrams / 1000 * 100, 100)}%` }}
              />
            </div>
          </div>
        </div>
            ...
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="text-right">
                <span className={`text-2xl font-bold tabular-nums ${isDark ? 'text-cyan-400' : 'text-cyan-600'}`}>
                  {formatO2(o2GeneratedLiters).value}
                </span>
                <span className={`text-sm ml-1 ${isDark ? 'text-cyan-500' : 'text-cyan-700'}`}>
                  {formatO2(o2GeneratedLiters).unit}
                </span>
              </div>
            </div>
            <div className={`h-2 rounded-full ${isDark ? 'bg-slate-700' : 'bg-gray-200'}`}>
              <div
                className="h-full rounded-full bg-gradient-to-r from-cyan-600 to-blue-500 transition-all"
                style={{ width: `${Math.min(o2GeneratedLiters / 10 * 100, 100)}%` }}
              />
            </div>
          </div>
        </div>
